...
/// 实现了内阴影的BoxShadow
class ZzBoxShadow extends painting.BoxShadow {
  const ZzBoxShadow({
  ...
  @override
  ZzBoxShadow scale(double factor) {
    return ZzBoxShadow(
      color: color,
  ...
  static ZzBoxShadow? lerp(ZzBoxShadow? a, ZzBoxShadow? b, double t) {
    if (a == null && b == null) {
      return null;
    }
    if (a == null) {
  ...
  ///
  /// {@macro dart.ui.shadow.lerp}
  static List<ZzBoxShadow>? lerpList(
    List<ZzBoxShadow>? a,
    List<ZzBoxShadow>? b,
    double t,
  ) {
    ...
    return <ZzBoxShadow>[
      for (int i = 0; i < commonLength; i += 1) ZzBoxShadow.lerp(a[i], b[i], t)!,
  ...
  @override
  String toString() =>
      'KqBoxShadow($color, $offset, ${debugFormatDouble(blurRadius)}, ${debugFormatDouble(spreadRadius)}), $blurStyle, $inset)';
}
